...
export function ClientPortalManager({
  ...
  const relativePortalPath = useMemo(() => `/portal/${token}`, [token]);
  const [portalUrl, setPortalUrl] = useState(relativePortalPath);

  useEffect(() => {
    setPortalUrl(`${window.location.origin}${relativePortalPath}`);
  }, [relativePortalPath]);
    ...
    <Card className="p-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-sm leading-6 text-slate-500">
            Share a secure read-only page with this client so they can view
            projects, findings, recommendations, and exported reports.
          </p>
        </div>
      ...
      <div className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-4">
        ...
        <div className="mt-2 flex flex-col gap-3 md:flex-row md:items-center">
          ...
          <div className="flex gap-2">
            ...
            <Button asChild variant="outline">
              <a href={relativePortalPath} target="_blank" rel="noreferrer">
        ...
        {!enabled && (
          <p className="mt-3 text-sm text-amber-700">
            This link is currently disabled. Enable the portal before sending it
            to a client.
          </p>
        )}
      </div>
      ...
      <div className="mt-6 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <p className="text-sm text-slate-500">
          {regeneratedAt
            ? `Last regenerated ${new Date(regeneratedAt).toLocaleDateString()}`
            : "This portal link has not been regenerated yet."}
        </p>
